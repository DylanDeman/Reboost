...
const PlaatsMemoized = memo(function Plaats({
  ...
  const handleDelete = useCallback(() => {
    if (!hasEvenementen) onDelete(id);
  }, [id, onDelete, hasEvenementen]);

  return (
    <div
      className={`card text-${textTheme} card-bg bg-${theme} bg-gradient border-0 h-100 d-flex flex-column shadow-sm`}
    >
      <div className="card-body d-flex flex-column p-4">
        <div className="d-flex align-items-center mb-3">
          <IoLocationOutline className="me-2 text-primary" size={20} />
          <h5 className="card-title mb-0 fw-bold" data-cy="plaats_naam">
            {naam}
          </h5>
        </div>
        ...
        <div className="flex-grow-1">
          <div className={`card-text mb-3`} data-cy="plaats_adres">
            <p className={`mb-1 fw-medium text-${textTheme}`}>
              {straat} {huisnummer}
            </p>
            <p className={`mb-0 text-${textTheme} opacity-75`}>
              {postcode} {gemeente}
            </p>
          </div>
        </div>
