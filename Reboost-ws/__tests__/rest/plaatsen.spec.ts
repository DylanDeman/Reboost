...
describe('Plaatsen', () => {
  ...
  describe('GET /api/plaatsen/:id/evenementen', () => {
    beforeAll(async () => {
      await clearDatabase();
    ...
    it('moet een 200 en de evenementen voor een plaats teruggeven', async () => {
      const response = await request
        .get(`${url}/1/evenementen`)
        .set(authHeader());
      ...
      // Plaats 1 has 3 evenementen in our test data
      expect(response.body.items.length).toBe(3);
      
      // Check for the expected properties in the returned evenementen
      response.body.items.forEach((evenement: any) => {
        expect(evenement).toHaveProperty('id');
        expect(evenement).toHaveProperty('naam');
        expect(evenement).toHaveProperty('datum');
        expect(evenement).toHaveProperty('auteur');
        expect(evenement).toHaveProperty('plaats');
        expect(evenement.plaats.id).toBe(1);
      });
    });

    it('moet een lege lijst teruggeven als de plaats geen evenementen heeft', async () => {
      // First delete all evenementen for plaats 1
      await prisma.evenement.deleteMany({ where: { plaats_id: 1 } });
    ...
    it('moet een lege lijst teruggeven als de plaats niet bestaat', async () => {
      const response = await request
        .get(`${url}/999/evenementen`)
        .set(authHeader());
      
      // Since we're getting evenementen by plaats_id, we should get an empty array
      // rather than an error for a non-existent plaats
      expect(response.status).toBe(200);
      expect(response.body.items).toEqual([]);
    });
  });
